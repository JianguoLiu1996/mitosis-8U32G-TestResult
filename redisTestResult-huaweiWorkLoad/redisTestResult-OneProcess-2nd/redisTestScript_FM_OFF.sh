...
function prepareData(){
	...
	memtier_benchmark -s ${SERVERADDR} \
		...
		--ratio=1:0 \
		--key-minimum=1 \
		--key-maximum=20971520 \
		--key-prefix=memtier-benchmark-prefix-redistests \
		--out-file=${OUTPUTPATH}redis_prepare_${CONFIG}_$(date +"%Y%m%d%H%M%S").log >> ${OUTPUTPATH}redis_prepare_process_${CONFIG}_$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
...
function Gauss82(){
	...
	memtier_benchmark -s ${SERVERADDR} \
		...
		--key-prefix=memtier-benchmark-prefix-redistests \
		--key-stddev=873847 \
		--out-file=${OUTPUTPATH}redis_result_Gauss82_${CONFIG}_${NUMBER}$(date +"%Y%m%d%H%M%S").log >> ${OUTPUTPATH}redis_process__Gauss82_${CONFIG}_${NUMBER}$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
...
function Gauss110(){
	...
	memtier_benchmark -s ${SERVERADDR} \
		...
		--key-prefix=memtier-benchmark-prefix-redistests \
		--key-stddev=873847 \
		--out-file=${OUTPUTPATH}redis_result_Gauss110_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log >> ${OUTPUTPATH}redis_process_Gauss110_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
...
function Random82(){
	...
	memtier_benchmark -s ${SERVERADDR} \
		...
		--key-maximum=20971520 \
		--key-prefix=memtier-benchmark-prefix-redistests \
		--out-file=${OUTPUTPATH}redis_result_Random82_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log >> ${OUTPUTPATH}redis_process_Random82_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
...
function Random110(){
	...
	memtier_benchmark -s ${SERVERADDR} \
		...
		--key-maximum=20971520 \
		--key-prefix=memtier-benchmark-prefix-redistests \
		--out-file=${OUTPUTPATH}redis_result_Random110_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log >> ${OUTPUTPATH}redis_process_Random110_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
...
function Sequential82(){
	...
	memtier_benchmark -s ${SERVERADDR} \
		...
		--key-maximum=20971520 \
		--key-prefix=memtier-benchmark-prefix-redistests \
		--out-file=${OUTPUTPATH}redis_result_Sequential82_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log >> ${OUTPUTPATH}redis_process_Sequential82_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
...
function Sequential110(){
	...
	memtier_benchmark -s ${SERVERADDR} \
		...
		--key-maximum=20971520 \
		--key-prefix=memtier-benchmark-prefix-redistests \
		--out-file=${OUTPUTPATH}redis_result_Sequential110_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log >> ${OUTPUTPATH}redis_process_Sequential110_${CONFIG}_${NUMBER}_$(date +"%Y%m%d%H%M%S").log
	wait
	sleep 1m
